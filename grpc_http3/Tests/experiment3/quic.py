...
def run(args):
    ...
    s2 = net['s2']
    s2.cmd('tc qdisc add dev s2-eth1 root netem delay {}ms rate {}Mbps loss random {}%'.format(args.delaybackup, args.bandwidthbackup, args.lossbackup))
    s2.cmd('tc qdisc add dev s2-eth2 root netem delay {}ms rate {}Mbps loss random {}%'.format(args.delaybackup, args.bandwidthbackup, args.lossbackup))
    server = net['h3']

    info('Starting server\n')
    server.cmd('cd .. ; ./server -s 10.1.0.1:8080 -p fileexchange --multipath --rcvd-threshold 25 --max-bidi-remote 10000 --scheduler lrtt --timeout 30000 > ./experiment3/server_out &')
    ...
    client.cmd('cd .. ; ./client -s 10.1.0.1:8080 -p fileexchange --multipath -A 10.0.0.1:5050 -A 10.0.1.1:3355 --file file -n 80 --rcvd-threshold 25 --time 200 --ack-eliciting-timer 10000000 --scheduler lrtt --timeout 30000 > {}'.format(args.file))
    sleep(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--file', type=str, help='Client output file', required=False, default='./experiment3/outputQuic')
